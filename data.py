...
def linearPredict(featureMat, weights, biases):
    ...
    """
    logitScores = np.array([np.empty([5]) for i in range(featureMat.shape[0])])
...
def softmaxNormalizer(logitMatrix):
    ...
    probabilities = np.array([np.empty([5]) for i in range(logitMatrix.shape[0])])
    # creating empty(garbage value) array for each feature set

    for i in range(logitMatrix.shape[0]):
        exp = np.exp(logitMatrix[i])
        # exponentiates each element of the logit array
        sumOfArr = np.sum(exp)
        # adds up all the values in the exponentiated array
...
def stochGradDes(learning_rate, epochs, target, features, weights, biases):
    ...
    for i in range(epochs):
        probabilities, _ = multinomialLogReg(features, weights, biases)
        ...
        loss_list = np.append(loss_list, CELoss)
        # Adds the CELoss value for the epoch to loss_list
